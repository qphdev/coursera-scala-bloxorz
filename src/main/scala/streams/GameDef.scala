...
trait GameDef {
  ...
  case class Pos(x: Int, y: Int) {
    ...
    def deltaRow(d: Int): Pos = copy(x = x + d)

    /** The position obtained by changing the `col` coordinate by `d` */
    def deltaCol(d: Int): Pos = copy(y = y + d)
  }
